...
-- Invite codes: authenticated users can read (for validation) and insert (for generation)
CREATE POLICY "invite_codes_select" ON public.invite_codes
  FOR SELECT USING (auth.uid() IS NOT NULL OR TRUE);
  -- Allow unauthenticated reads for signup validation
...
CREATE POLICY "invite_codes_update" ON public.invite_codes
  ...
  -- Update needed for marking as used during signup
